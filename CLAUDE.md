...
7. **contact.html** — Nous écrire : formulaire de contact accessible (nom, ville d'habitation, e-mail et/ou téléphone, profil facultatif, message, consentement RGPD avec lien vers la page confidentialité). **Activé depuis juillet 2026** (`FORMULAIRE_ACTIF = true` dans `js/contact.js`, réception à `gemtsaistres@gmail.com` (boîte du GEM créée le 23/09/2026, transférée vers clairegazeau@yahoo.com ; avant cette date : clairegazeau@yahoo.com)), une fois les dates et le lieu des premières rencontres fixés : le bandeau annonce que le formulaire est ouvert et le bouton d'envoi est actif. Fonctionnement et procédure décrits dans `CONFIGURATION-FORMULAIRE.md` (section 9).
...
- ~~Nom de l'association et logo.~~ **CaméléOn** + logo au caméléon affichés depuis le 19 septembre 2026 (EAU'TYSM du 12 au 19 septembre), en attente du vote du 7 novembre (voir § 0). Le nom reste à vérifier au **RNA** et à l'**INPI** après le vote.
...
- **Activation** = deux valeurs en haut de `js/contact.js` : `FORMULAIRE_ACTIF` et `EMAIL_RECEPTION` (adresse qui reçoit les demandes). **Fait en juillet 2026** : `FORMULAIRE_ACTIF = true` et réception à `gemtsaistres@gmail.com` (boîte du GEM créée le 23/09/2026, transférée vers clairegazeau@yahoo.com ; avant cette date : clairegazeau@yahoo.com) ; le bandeau annonce désormais que le formulaire est ouvert. Procédure : `CONFIGURATION-FORMULAIRE.md`.
